...
if(!isValidString(data.title)) return res.status(400).send({status:false,message:"Enter title in string form"});
if(data.title.length<3) return res.status(400).send({status:false,message:"Can contain only minimum 3 letters"});
let titlePresent = await bookModel.findOne({title:data.title})
if(titlePresent) return res.status(400).send({status:false,message:"Title is already present"});

if(!data.excerpt)  return res.status(400).send({status:false,message:"Provide excerpt"});
if(!isValidString(data.excerpt)) return res.status(400).send({status:false,message:"Enter excerpt in string form"});
if(data.excerpt.length<3) return res.status(400).send({status:false,message:"Must contain minimum 3 letters"});


if(!data.ISBN)  return res.status(400).send({status:false,message:"Provide ISBN"});
if(!isValidString(data.ISBN)) return res.status(400).send({status:false,message:"Enter ISBN in string"});


if(data.ISBN.length!=10 && data.ISBN.length!=13) return res.status(400).send({status:false,message:"ISBN must be of length 10 or 13"});
...
if(!isValidString(data.category)) return res.status(400).send({status:false,message:"Enter category in string"});

if(!data.subcategory) return res.status(400).send({status:false,message:"Provide subcategory"});
if(!isValidString(data.subcategory)) return res.status(400).send({status:false,message:"Enter subcategory in string"});
...
return res.status(400).send({status:false,message:"Please enter a valid date(YYYY-MM-DD)"})
